...
def train(x, cond, modules, optimizer, kl_anneal, args, device):
    ...
    for i in range(1, args.n_past + args.n_future):
        ...
        z, mu, logvar = modules['posterior'](h_seq[i][0])
        # TODO : embedding condition
        g = modules['frame_predictor'](torch.cat((h, z, cond[:, i - 1]), 1))
...
def plot_pred(validate_seq, validate_cond, modules, epoch, args, device):
    ...
    for i in range(pred_seq.shape[0]):
        ...
        imageio.mimsave(f'{dir}batch_size\={i}.gif', frames, duration=20)
...
def main():
    ...
    if args.model_dir != '':
        ...
        name = 'rnn_size=%d-predictor-posterior-rnn_layers=%d-%d-n_past=%d-n_future=%d-lr=%.4f-g_dim=%d-z_dim=%d-last_frame_skip=%s-beta=%.7f-anneal_cycle=%d-anneal_ratio=%.2f-tfr_decay_step=%.3f'\
