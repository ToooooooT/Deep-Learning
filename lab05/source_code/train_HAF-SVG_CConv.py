...
from models.vgg_64_CConv import vgg_decoder, vgg_encoder
...
def train(x, cond, modules, optimizer, kl_anneal, args, device):
    ...
    x = x.to(device)
    cond = cond.to(device)
    h_seq = [modules['encoder'](x[:, i], cond[:, i]) for i in range(args.n_past + args.n_future + 1)]
    ...
    for i in range(1, args.n_past + args.n_future):
        ...
        if i <= args.n_past or use_teacher_forcing:
            ...
            h, skip = modules['encoder'](y_pred, cond[:, i - 1])
        ...
        y_pred = modules['decoder']([g, skip], cond[:, i - 1])
...
def main():
    ...
    if args.model_dir != '':
        ...
        name = 'CCONV-rnn_size=%d-predictor-posterior-rnn_layers=%d-%d-n_past=%d-n_future=%d-lr=%.4f-g_dim=%d-z_dim=%d-last_frame_skip=%s-beta=%.7f-anneal_cycle=%d-anneal_ratio=%.2f-tfr_decay_step=%.3f'\
    ...
    if args.model_dir != '':
        ...
        encoder = vgg_encoder(args.g_dim, args.c_dim)
        decoder = vgg_decoder(args.g_dim, args.c_dim)
