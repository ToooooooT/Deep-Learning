...
class DDPG:
    def __init__(self, args, num_inputs, action_space):
        ...
        self._critic_net = CriticNet(state_dim=num_inputs, action_dim=action_space.shape[0]).to(args.device)
        # target network
        self._target_actor_net = ActorNet(action_space, state_dim=num_inputs, action_dim=action_space.shape[0]).to(args.device)
        self._target_critic_net = CriticNet(state_dim=num_inputs, action_dim=action_space.shape[0]).to(args.device)
    ...
    def _update_behavior_network(self, gamma):
        ...
        critic_loss = criterion(q_value, q_target.detach())
        # optimize critic
        actor_net.zero_grad()
...
def main():
    ...
    parser.add_argument('-m', '--model', default='pretrained/ddpg.pth')
